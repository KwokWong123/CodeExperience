...
function ViewPicker({
  value, idx, currentStage, hasRun, liveActive, onChange, libraryArtifacts, customCharts,
}: {
  value: PanelView; idx: number; currentStage: number; hasRun: boolean; liveActive: boolean;
  ...
  const isAvailable = (v: PanelView) => {
    ...
    return (hasRun && currentStage >= stage) || (liveActive && stage >= 4);
  };
...
function resolvePanel(
  ...
  if (sourceView === 'live') {
    // The 'live' panel always shows stage 4 data when liveActive — stage 4 is the only
    // stage where BOTH chart AND table are fully recomputed by computeLiveStageResults.
    // Stage 5's chart is overridden by RecipeScatterPlot (static data), and stage 5's
    // table is never recomputed, so anchoring to stage 4 ensures changes are visible.
    const liveStage = liveActive ? 4 : currentStage;
    return { chart: stageResults[liveStage]?.chart ?? null, table: stageResults[liveStage]?.table ?? null, isLoading, hasRun, stage: liveStage };
  }
