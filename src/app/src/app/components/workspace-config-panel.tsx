...
const MODEL_STATUS_CONFIG: Record<string, { dot: string; label: string; color: string; bg: string }> = {
...
export function WorkspaceConfigPanel({ objectives, isLocked = false, recipesGenerated = false, isGenerating = false, onGenerate }: WorkspaceConfigPanelProps) {
  ...
  const handleTargetToggle = (id: string) =>
    setLocalObjectives((prev) =>
      prev.map((o) =>
        o.id === id
          ? { ...o, target: o.target === 'maximize' ? 'minimize' : 'maximize' }
          : o
      )
    );
  ...
  return (
    <div className="h-full bg-white border-r border-gray-200 flex flex-col overflow-hidden">
      {/* Header */}
      <div className="px-3 py-2.5 border-b border-gray-200 bg-gradient-to-r from-[#3F98FF]/5 to-transparent shrink-0">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 bg-[#3F98FF]/10 rounded-md flex items-center justify-center shrink-0">
            <SlidersHorizontal className="w-3.5 h-3.5 text-[#3F98FF]" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="text-[11px] font-bold text-gray-900 leading-none">Workspace Config</div>
            <div className="text-[9px] text-gray-400 mt-0.5 truncate">{selectedProject?.name}</div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* ── Model ───────────────────────────────────────────── */}
        <Section title="Model" icon={<BrainCircuit className="w-3.5 h-3.5" />} defaultOpen={true}>
          <SelectRow
            value={config.modelId}
            onChange={(v) => setConfig((c) => ({ ...c, modelId: v, modelVersion: MODELS.find((m) => m.id === v)?.defaultVersion || 'v1.0' }))}
          ...
          {selectedModel && (
            <div className="mt-2 space-y-2">
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-[9px] text-gray-500 font-medium">{selectedModel.type}</span>
                <span className="text-gray-300">·</span>
                <span className="text-[9px] text-gray-400 truncate">↳ {selectedModel.project}</span>
              </div>

              <div>
                <div className="flex items-center gap-1 mb-1.5">
                  <GitBranch className="w-2.5 h-2.5 text-gray-400" />
                  <span className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider">Version / Training Data</span>
                </div>
                {(() => {
                  ...
                  return (
                    <div className="space-y-1.5">
                      {selectedModel.versions.map((ver, idx) => {
                        ...
                        const isSelected = config.modelVersion === ver.version;
                        const isLatest = idx === 0;
                        const prevSamples = idx < selectedModel.versions.length - 1 ? selectedModel.versions[idx + 1].trainSamples : 0;
                        const addedSamples = ver.trainSamples - prevSamples;
                        const pct = Math.round((ver.trainSamples / maxSamples) * 100);
                        return (
                          <button
                            key={ver.version}
                            onClick={() => setConfig((c) => ({ ...c, modelVersion: ver.version }))}
                            className={`w-full text-left rounded-lg border p-2 transition-all ${
                              isSelected ? 'border-[#3F98FF] bg-[#3F98FF]/5 shadow-sm' : 'border-gray-100 bg-gray-50 hover:border-gray-300 hover:bg-white'
                            }`}
                          >
                            <div className="flex items-center gap-1.5 mb-1">
                              <span className={`text-[10px] font-bold font-mono ${isSelected ? 'text-[#3F98FF]' : 'text-gray-700'}`}>{ver.version}</span>
                              {isLatest && <span className="text-[8px] font-semibold px-1 py-0.5 rounded bg-[#3F98FF]/10 text-[#3F98FF]">latest</span>}
                              <span className={`ml-auto flex items-center gap-0.5 text-[8px] font-semibold px-1.5 py-0.5 rounded-full ${vCfg.color} ${vCfg.bg}`}>
                                <span className={`w-1 h-1 rounded-full ${vCfg.dot}`} />{vCfg.label}
                              </span>
                            </div>
                            <div className="flex items-center gap-1.5 mb-1">
                              <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, backgroundColor: isSelected ? '#3F98FF' : '#9ca3af' }} />
                              </div>
                              <span className={`text-[9px] font-bold shrink-0 ${isSelected ? 'text-[#3F98FF]' : 'text-gray-500'}`}>{ver.trainSamples.toLocaleString()}</span>
                              {addedSamples > 0 && <span className="text-[8px] text-green-600 font-semibold shrink-0">+{addedSamples.toLocaleString()}</span>}
                            </div>
                            <div className="flex items-start justify-between gap-1">
                              <span className="text-[9px] text-gray-400 leading-tight">{ver.notes}</span>
                              <span className="text-[8px] text-gray-300 shrink-0">{ver.date}</span>
                            </div>
                            {isSelected && ver.status !== 'training' && ver.r2 != null && (
                              <div className="grid grid-cols-3 gap-1 mt-1.5 pt-1.5 border-t border-[#3F98FF]/10">
                                {[
                                  { label: 'R²', value: ver.r2.toFixed(3), color: ver.r2 >= 0.90 ? '#059669' : '#3F98FF' },
                                  { label: 'RMSE', value: ver.rmse != null ? ver.rmse.toFixed(2) : '—', color: '#7c3aed' },
                                  { label: 'Features', value: selectedModel.features, color: '#3F98FF' },
                                ].map((m) => (
                            ...
                            {isSelected && ver.status === 'training' && (
                              <div className="flex items-center gap-1 mt-1.5 pt-1.5 border-t border-amber-100">
                                <span className="w-1.5 h-1.5 bg-amber-400 rounded-full animate-pulse" />
                                <span className="text-[9px] text-amber-600">Training in progress — not available for inference</span>
                              </div>
                            )}
                          </button>
                        );
              ...
              {(() => {
                const ds = DATASET_DETAILS[selectedModel.datasetId];
                if (!ds) return null;
                const versionDataset = selectedVersion
                  ? { ...ds, rows: selectedVersion.trainSamples, description: `Training snapshot for ${selectedModel.label} ${selectedVersion.version} — ${selectedVersion.trainSamples.toLocaleString()} samples used during model fitting. ${ds.description}` }
                  : ds;
                return (
                  <button
                    onClick={() => setViewingDataset(versionDataset)}
                    className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-md border border-[#3F98FF]/30 text-[10px] text-[#3F98FF] hover:bg-[#3F98FF]/5 hover:border-[#3F98FF] transition-colors font-medium"
                  >
                    <Eye className="w-3 h-3" />
                    View Training Dataset
                    {selectedVersion && (
                      <span className="ml-0.5 text-[8px] bg-[#3F98FF]/10 px-1 py-0.5 rounded font-semibold">
                        {selectedVersion.trainSamples.toLocaleString()} rows
                      </span>
                    )}
                  </button>
                );
              })()}
              {staleModel && <StalePill label="Model / version" />}
            </div>
          )}
        </Section>

        {/* ── Project ─────────────────────────────────────────── */}
        <Section title="Project" icon={<FolderOpen className="w-3.5 h-3.5" />} defaultOpen={false}>
          <SelectRow
            value={config.projectId}
            onChange={(v) => setConfig((c) => ({ ...c, projectId: v }))}
            options={PROJECTS.map((p) => ({ value: p.id, label: p.name }))}
          />
          {selectedProject && (
            <div className="mt-1.5 flex items-center gap-1.5">
              <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${
                selectedProject.status === 'active' ? 'bg-blue-50 text-[#3F98FF]' :
                selectedProject.status === 'complete' ? 'bg-green-50 text-green-600' :
                'bg-gray-100 text-gray-500'
              }`}>
                {selectedProject.status === 'active' ? '● Live' :
                 selectedProject.status === 'complete' ? '✓ Complete' : '○ Draft'}
              </span>
              <span className="text-[9px] text-gray-400">ID: PRJ-00{config.projectId}</span>
            </div>
          )}
          {staleProject && <StalePill label="Project" />}
        </Section>
            ...
            <div className="space-y-2">
              ...
              {localObjectives.map((obj) => (
                <ObjectiveSlider
                  key={obj.id}
                  obj={obj}
                  weight={objectiveWeights[obj.id] ?? PRIORITY_DEFAULT_WEIGHTS[obj.priority] ?? 50}
        ...
        <Section title="Constraints" icon={<ShieldCheck className="w-3.5 h-3.5" />} defaultOpen={false} badge={`${constraints.length}`}>
          <div className="space-y-1">
            {constraints.map((c) => (
              <div
                key={c.id}
                className="group flex items-center gap-1.5 bg-gray-50 rounded-md px-2 py-1.5 hover:bg-gray-100 transition-colors"
              >
                <span className={`text-[8px] font-semibold px-1 py-0.5 rounded shrink-0 ${TYPE_CONFIG[c.type].color}`}>
                  {TYPE_CONFIG[c.type].label}
                </span>
                <div className="flex-1 min-w-0">
                  <div className="text-[9px] font-semibold text-gray-700 truncate">{c.ingredient}</div>
                ...
                <button
                  ...
                  className="w-4 h-4 flex items-center justify-center rounded opacity-0 group-hover:opacity-100 hover:bg-red-100 text-red-400 transition-all shrink-0"
                >
      ...
      <div className="px-3 py-3 border-t border-gray-200 shrink-0 bg-white">
        ...
        {anyStale && (
          <div className="flex items-center gap-1.5 mb-2 px-2 py-1.5 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <span className="text-[9px] text-amber-700 leading-snug">
              Config changed since last run — <span className="font-semibold">recipes may be outdated</span>
            </span>
          </div>
        )}
        <button
        ...
        {recipesGenerated && !anyStale && !isGenerating && (
