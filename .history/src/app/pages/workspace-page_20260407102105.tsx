...
export function WorkspacePage() {
  ...
  const objectiveScatterBounds = useMemo(() => {
    const bounds: {
      spf?: { target?: number; min?: number; max?: number };
      wr?: { target?: number; min?: number; max?: number };
      stability?: { target?: number; min?: number; max?: number };
      score?: { target?: number; min?: number; max?: number };
    } = {};

    const toAxis = (attribute: string): 'spf' | 'wr' | 'stability' | 'score' | null => {
      const key = attribute.toLowerCase();
      if (key.includes('spf')) return 'spf';
      if (key.includes('water')) return 'wr';
    ...
    objectives.forEach((obj) => {
      const axis = toAxis(obj.attribute);
      if (!axis) return;
      bounds[axis] = {
        target: parseTargetNumber(obj.value) ?? undefined,
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="flex-1 overflow-hidden min-w-0 flex flex-col">
          <WorkspaceCanvas
            ...
            objectiveBounds={objectiveScatterBounds}
...
function renderInlineFormatting(text: string) {
  ...
  return parts.map((part, idx) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return <code key={idx} className="rounded bg-gray-100 px-1 py-0.5 text-[10px] text-gray-700">{part.slice(1, -1)}</code>;
    }
    return <span key={idx}>{part}</span>;
  });
}

function AssistantFormattedMessage({ content }: { content: string }) {
  ...
  while (i < lines.length) {
    ...
    if (h3) {
      nodes.push(<h3 key={key++} className="text-[11px] font-semibold text-gray-900 mt-1.5 mb-1">{renderInlineFormatting(h3[1])}</h3>);
      i += 1;
      continue;
    }
    ...
    if (h2) {
      nodes.push(<h2 key={key++} className="text-[12px] font-bold text-gray-900 mt-1.5 mb-1">{renderInlineFormatting(h2[1])}</h2>);
      i += 1;
      continue;
    }
    ...
    if (h1) {
      nodes.push(<h1 key={key++} className="text-[13px] font-bold text-gray-900 mt-1.5 mb-1">{renderInlineFormatting(h1[1])}</h1>);
      i += 1;
      continue;
    }
    ...
    if (/^[-*]\s+/.test(line)) {
      ...
      while (i < lines.length && /^\s*[-*]\s+/.test(lines[i])) {
        ...
        items.push(<li key={key++} className="leading-relaxed">{renderInlineFormatting(text)}</li>);
        i += 1;
      }
      nodes.push(<ul key={key++} className="list-disc pl-4 space-y-0.5 my-1.5 text-gray-800">{items}</ul>);
      continue;
    }
    ...
    if (/^\d+\.\s+/.test(line)) {
      ...
      nodes.push(<ol key={key++} className="list-decimal pl-4 space-y-0.5 my-1.5 text-gray-800">{items}</ol>);
      continue;
    }

    nodes.push(<p key={key++} className="text-gray-800 leading-relaxed">{renderInlineFormatting(line)}</p>);
    i += 1;
  }
