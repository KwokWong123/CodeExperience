...
const PROJECT_MODELS: Record<string, {
  ...
  '1': [
    { id: 'mdl-1', name: 'SPF-XGB-Ensemble', version: 'v3.0', type: 'XGBoost + Random Forest', framework: 'scikit-learn', dataset: 'Sunscreen Formulation Database', status: 'deployed', r2: 0.940, rmse: 2.31, trainSamples: 1957, features: 18, inferenceMs: 12, lastRetrained: 'Mar 14, 2026', description: 'Ensemble model predicting SPF protection factor from 18 formulation features. Production model.', tags: ['ensemble', 'SPF', 'production'] },
    { id: 'mdl-2', name: 'SPF-XGB-Ensemble', version: 'v2.1', type: 'XGBoost + Random Forest', framework: 'scikit-learn', dataset: 'Sunscreen Formulation Database', status: 'archived', r2: 0.912, rmse: 3.04, trainSamples: 1600, features: 15, inferenceMs: 11, lastRetrained: 'Jan 22, 2026', description: 'Previous ensemble model. Archived after v3.0 deployment.', tags: ['ensemble', 'SPF', 'archived'] },
    { id: 'mdl-3', name: 'WR-Stability-RF', version: 'v1.2', type: 'Random Forest', framework: 'scikit-learn', dataset: 'Sunscreen Formulation Database', status: 'staging', r2: 0.881, rmse: 1.82, trainSamples: 1957, features: 18, inferenceMs: 8, lastRetrained: 'Mar 10, 2026', description: 'Jointly predicts water resistance and stability index from formulation inputs.', tags: ['stability', 'water resistance', 'staging'] },
  ],
  '2': [
    { id: 'mdl-4', name: 'CMC-GNN-v2', version: 'v2.0', type: 'Graph Neural Network', framework: 'PyTorch Geometric', dataset: 'Surfactant CMC Dataset', status: 'deployed', r2: 0.887, rmse: 0.42, trainSamples: 1456, features: 22, inferenceMs: 45, lastRetrained: 'Feb 10, 2026', description: 'Graph neural network model for CMC prediction using molecular topology.', tags: ['GNN', 'CMC', 'graph'] },
    { id: 'mdl-5', name: 'CMC-MLP-Baseline', version: 'v1.0', type: 'Multi-Layer Perceptron', framework: 'scikit-learn', dataset: 'Surfactant CMC Dataset', status: 'archived', r2: 0.821, rmse: 0.71, trainSamples: 1200, features: 18, inferenceMs: 3, lastRetrained: 'Oct 14, 2025', description: 'Baseline MLP. Replaced by GNN approach.', tags: ['MLP', 'CMC', 'baseline'] },
  ],
  '3': [
    { id: 'mdl-6', name: 'SensoryScore-RF', version: 'v1.1', type: 'Random Forest + SHAP', framework: 'scikit-learn', dataset: 'Emollient Sensory Panel Results', status: 'staging', r2: 0.912, rmse: 0.38, trainSamples: 356, features: 14, inferenceMs: 5, lastRetrained: 'Mar 5, 2026', description: 'Predicts composite sensory score from emollient blend parameters.', tags: ['sensory', 'random forest', 'SHAP'] },
  ],
  '4': [], '5': [],
};
...
const PROJECT_WORKSPACES_BASE: Record<string, WorkspaceSummary[]> = {
  '1': [
    {
      id: '1',
      name: 'SPF Optimization — Run #4',
      description: 'Bayesian optimization across SPF, WR, and cost with top candidates selected.',
      status: 'active',
      columns: 2,
      panelCount: 4,
    ...
    },
    {
      ...
      description: 'Initial exploratory analysis and feature correlation audit for the sunscreen dataset.',
      status: 'archived',
      columns: 2,
      panelCount: 3,
  ...
  '2': [
    {
      ...
      description: 'Virtual screening pass for surfactant candidates using CMC-GNN-v2.',
      status: 'saved',
      columns: 3,
      panelCount: 2,
  ...
  '3': [
    {
      ...
      description: 'Sensory and rheology-driven blend exploration with SHAP contribution analysis.',
      status: 'saved',
      columns: 2,
      panelCount: 3,
  ...
  '4': [
    {
      ...
      description: 'Photostability degradation modelling for UV filter + stabilizer combinations.',
      status: 'active',
      columns: 2,
      panelCount: 3,
      lastOpened: 'Today',
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-[10px] text-gray-500 mt-0.5">Dataset: {mdl.dataset}</div>
          </div>
        </div>
...
type Tab = 'overview' | 'datasets' | 'models' | 'analysis' | 'reports' | 'workspace';
...
export function ProjectDetailPage() {
  ...
  const customWorkspaceCount = (() => {
    try {
      const raw = localStorage.getItem('noble-ai-custom-workspaces');
      const custom = raw ? (JSON.parse(raw) as Array<{ projectId?: string | null }>) : [];
      return custom.filter((w) => w.projectId === id).length;
    } catch {
      return 0;
    }
  })();
  ...
  const TABS: { id: Tab; label: string; icon: React.ElementType; count: number }[] = [
    ...
    { id: 'workspace', label: 'Workspace', icon: ExternalLink, count: workspaceCount },
  ];
        ...
        {activeTab === 'datasets' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => navigate('/datasets', { state: { openUpload: true, projectName: project.name } })}
        ...
        {activeTab === 'models' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={() => navigate('/models', { state: { openRegister: true } })}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-[#3F98FF] text-white rounded-lg font-medium hover:bg-[#2563eb] transition-colors"
              >
