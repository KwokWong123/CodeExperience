...
import {
  X, BarChart2, TrendingUp, Activity, Crosshair,
...
export const DATASOURCE_FIELDS: Record<DataSourceKey, {
  ...
  'stage-4': {
    ...
    xLabel: 'Candidate ID',
    yOptions: [
      { key: 'score',     name: 'Composite Score',   color: '#3F98FF' },
      { key: 'spf',       name: 'Predicted SPF',     color: '#7c3aed' },
      { key: 'wr',        name: 'Water Resistance %', color: '#059669' },
      { key: 'stability', name: 'Stability %',        color: '#f59e0b' },
      { key: 'cost',      name: 'Cost Index',         color: '#ec4899' },
      { key: 'zno',       name: 'Zinc Oxide %',       color: '#06b6d4' },
      { key: 'tio2',      name: 'Titanium Dioxide %', color: '#84cc16' },
      { key: 'avo',       name: 'Avobenzone %',       color: '#f97316' },
      { key: 'oct',       name: 'Octocrylene %',      color: '#a78bfa' },
    ],
  },
  'stage-5': {
    label: 'Top Recipes',
...
const CHART_TYPES = [
  ...
  { key: 'scatter', label: 'Scatter', icon: <Crosshair   className="w-4 h-4" /> },
] as const;
...
export function ChartBuilderModal({
  ...
  const canSave = name.trim().length > 0 && yOptions.length > 0
    && (chartType !== 'scatter' || selectedY.length >= 2);
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="w-[340px] shrink-0 flex flex-col overflow-y-auto border-r border-gray-100 bg-gray-50/40">
            <div className="p-5 space-y-5">
              ...
              <div>
                <label className="block text-[11px] font-semibold text-gray-600 uppercase tracking-wider mb-1.5">
                  {chartType === 'scatter' ? 'Axes to Plot' : 'Series to Plot'}
                  <span className="ml-1 font-normal text-gray-400">
                    {chartType === 'scatter' ? '(select exactly 2)' : '(select one or more)'}
                  </span>
                </label>
                {chartType === 'scatter' && (
                  <div className="mb-2 px-2.5 py-1.5 bg-blue-50 border border-[#3F98FF]/20 rounded-md text-[10px] text-[#3F98FF] leading-relaxed">
                    1st selected = X axis · 2nd selected = Y axis
                  </div>
                )}
                {yOptions.length === 0 ? (
