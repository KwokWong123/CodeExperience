...
function RecipeScatterPlot() {
  ...
  const data = ALL_RECIPES.map((r) => ({ ...r }));

  const CustomDot = (props: any) => {
    const { cx, cy, payload } = props;
    if (!cx || !cy) return null;
    const r = payload as typeof ALL_RECIPES[0];
  ...
  const LEGEND_ITEMS = ALL_RECIPES.filter((r) => r.isTop3 || r.id === 'Baseline');
...
/* ── Generic field dropdown (for scatter axis selection) ─────── */

function FieldSelect({
  label, value, onChange, exclude, options,
}: {
  label: string;
  value: string;
  ...
  return (
    <div className="relative">
      <div className="text-[9px] text-gray-400 mb-0.5 uppercase tracking-wider font-semibold">{label}</div>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-md px-2 py-1 text-[11px] font-semibold text-gray-700 hover:bg-gray-100 transition-colors"
      >
      ...
      {open && (
        <div className="absolute top-full mt-1 left-0 z-50 bg-white border border-gray-200 rounded-lg shadow-xl py-1 min-w-[160px]">
          {options.filter((o) => o.key !== exclude).map((opt) => (
            <button
              key={opt.key}
              onClick={() => { onChange(opt.key); setOpen(false); }}
              className={`w-full text-left px-3 py-1.5 text-[11px] transition-colors flex items-center gap-2 ${opt.key === value ? 'bg-[#3F98FF]/8 text-[#3F98FF] font-semibold' : 'text-gray-700 hover:bg-gray-50 font-medium'}`}
            >
              {opt.color && <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: opt.color }} />}
              {opt.label}
              {opt.key === value && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-[#3F98FF]" />}
            </button>
          ))}
        </div>
      )}
...
/* ── Scatter chart driven by ChartConfig ─────────────────────── */

function ResultScatterChart({ config }: { config: ChartConfig }) {
  const fields = config.yAxisKeys;
  const [xField, setXField] = useState(fields[0]?.key ?? '');
  const [yField, setYField] = useState(fields[1]?.key ?? fields[0]?.key ?? '');
  const [selected, setSelected] = useState<string>(config.highlightKey ?? '');

  // Re-sync when config fields change (e.g. weights update)
  useEffect(() => {
    setXField(fields[0]?.key ?? '');
    setYField(fields[1]?.key ?? fields[0]?.key ?? '');
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fields.map((f) => f.key).join(',')]);

  useEffect(() => {
    if (config.highlightKey) setSelected(config.highlightKey);
  }, [config.highlightKey]);

  if (fields.length < 2) {
    return (
      <div className="flex items-center justify-center h-full text-[11px] text-gray-400 text-center px-8">
        Select at least 2 fields in the chart builder to use scatter mode.
      </div>
    );
  }

  const xMeta = fields.find((f) => f.key === xField) ?? fields[0];
  const yMeta = fields.find((f) => f.key === yField) ?? fields[1];

  const xVals = config.data.map((d: any) => Number(d[xField])).filter((v) => !isNaN(v));
  const yVals = config.data.map((d: any) => Number(d[yField])).filter((v) => !isNaN(v));
  const pad = (arr: number[]) => {
    const lo = Math.min(...arr); const hi = Math.max(...arr);
    const p = (hi - lo) * 0.1 || 1;
    return [Math.max(0, lo - p), hi + p] as [number, number];
  };

  const fieldOptions = fields.map((f) => ({ key: f.key, label: f.name, color: f.color }));

  const ScatterDot = (props: any) => {
    const { cx, cy, payload } = props;
    if (!cx || !cy) return null;
    const id = String(payload?.[config.xAxisKey] ?? '');
    const isSelected = id === selected;
    const isHighlighted = id === config.highlightKey;
    const r = isSelected ? 10 : isHighlighted ? 8 : 5;
    const color = xMeta.color;
    return (
      <g onClick={() => setSelected(id)} style={{ cursor: 'pointer' }} pointerEvents="all">
        <circle cx={cx} cy={cy} r={r + 8} fill="transparent" />
        {isSelected && (
          <circle cx={cx} cy={cy} r={r + 5} fill={color} fillOpacity={0.15}
            stroke={color} strokeWidth={1.5} strokeDasharray="3 2" />
        )}
        <circle cx={cx} cy={cy} r={r}
          fill={color} fillOpacity={isHighlighted || isSelected ? 1 : 0.5}
          stroke={isHighlighted || isSelected ? 'white' : 'none'} strokeWidth={1.5} />
        {(isHighlighted || isSelected) && (
  ...
  const ScatterTip = ({ active, payload }: any) => {
    ...
    const d = payload[0]?.payload;
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-2.5 text-[11px] min-w-[130px]">
        <div className="font-semibold text-gray-800 mb-1.5 pb-1 border-b border-gray-100">
          {d?.[config.xAxisKey]}
        </div>
        <div className="space-y-0.5">
          <div className="flex justify-between gap-4">
            <span className="text-gray-400">{xMeta.name}</span>
            <span className="font-semibold">{typeof d?.[xField] === 'number' ? d[xField].toFixed(1) : d?.[xField]}</span>
          </div>
          <div className="flex justify-between gap-4">
            <span className="text-gray-400">{yMeta.name}</span>
            <span className="font-semibold">{typeof d?.[yField] === 'number' ? d[yField].toFixed(1) : d?.[yField]}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="shrink-0 px-3 pt-2 pb-2 border-b border-gray-100 flex items-end gap-3">
        <FieldSelect label="X Axis" value={xField} onChange={setXField} exclude={yField} options={fieldOptions} />
        <span className="text-gray-300 pb-1 text-sm">vs</span>
        <FieldSelect label="Y Axis" value={yField} onChange={setYField} exclude={xField} options={fieldOptions} />
      </div>
      <div className="flex-1 min-h-0 px-2 pb-3">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 24, right: 40, bottom: 32, left: 10 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
            <XAxis type="number" dataKey={xField} domain={pad(xVals)}
              tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false}>
              <Label value={xMeta.name} offset={-12} position="insideBottom"
                style={{ fontSize: 10, fill: '#9ca3af' }} />
            </XAxis>
            <YAxis type="number" dataKey={yField} domain={pad(yVals)}
              tick={{ fontSize: 11, fill: '#9ca3af' }} axisLine={false} tickLine={false} width={40}>
              <Label value={yMeta.name} angle={-90} position="insideLeft" offset={10}
                style={{ fontSize: 10, fill: '#9ca3af' }} />
            </YAxis>
            <Tooltip content={<ScatterTip />} cursor={{ strokeDasharray: '3 3' }} />
            {config.referenceLines?.map((rl) => (
              <ReferenceLine key={rl.label} y={rl.y} stroke={rl.color} strokeDasharray="4 3"
                label={{ value: rl.label, fill: rl.color, fontSize: 10 }} />
            ))}
            <Scatter data={config.data} shape={<ScatterDot />} isAnimationActive={false} />
          </ScatterChart>
        </ResponsiveContainer>
      </div>
    </div>
...
export function ResultChart({ config }: { config: ChartConfig }) {
  // Scatter has its own full renderer with axis selectors
  if (config.type === 'scatter') return <ResultScatterChart config={config} />;
...
function ResultTable({ config }: { config: TableConfig }) {
  ...
  const renderCellValue = (value: any, column: ColumnDef) => {
    if (column.badge) {
...
export function WorkspaceResults({ chart, table, displayMode = 'chart', isLoading, hasRun, stage }: WorkspaceResultsProps) {
  if (isLoading) {
  ...
  if (displayMode === 'table') {
    ...
    return <ResultTable config={table} />;
  }
  ...
  return (
    <div className="h-full flex flex-col bg-white overflow-hidden">
      ...
      <div className="flex-1 min-h-0 px-4 py-3">
        <ResultChart key={JSON.stringify(chart.data[0])} config={chart} />
      </div>
    </div>
  );
}