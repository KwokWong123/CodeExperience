...
import {
  ...
  Plus, BarChart2, Table2, FileSpreadsheet, Zap, X, FileUp
} from 'lucide-react';
...
type ActiveTab = 'schema' | 'preview' | 'prepare' | 'quality';
const UPLOADABLE_FORMATS = ['csv', 'xlsx', 'xls', 'parquet', 'json'];
const UPLOAD_DOMAIN_COLORS: Record<string, string> = {
  'UV Protection': '#3F98FF',
  'Surface Chemistry': '#7c3aed',
  'Sensory Science': '#059669',
  Photochemistry: '#ea580c',
  General: '#64748b',
};

const getUploadFormat = (filename: string) => {
  const ext = filename.split('.').pop()?.toLowerCase() ?? '';
  return UPLOADABLE_FORMATS.includes(ext) ? ext : '';
};
...
const UPLOAD_PREVIEW_ROW_LIMIT = 5;
...
const getCleaningQualityDelta = (config: CleaningConfig) =>
  (config.imputeMissing ? 2.8 : 0) +
  (config.removeDuplicates ? 1.4 : 0) +
  (config.handleOutliers ? 1.9 : 0) +
  (config.normalizeNumeric ? 1.2 : 0) +
  (config.encodeCategoricals ? 1.2 : 0);
...
const derivePrepMetrics = (dataset: Dataset, config: CleaningConfig) => {
  const avgMissingPct = dataset.schema.length
    ? dataset.schema.reduce((sum, column) => sum + (100 - column.completeness), 0) / dataset.schema.length
    : 0;

  const approxMissingRows = Math.round(dataset.rows * (avgMissingPct / 100) * 0.22);
  const duplicateRows = Math.round(dataset.rows * (dataset.status === 'warning' ? 0.024 : 0.008));
  const outlierRows = Math.round(dataset.rows * (dataset.status === 'warning' ? 0.03 : 0.012));

  let estimatedRemainingMissing = approxMissingRows;
  if (config.imputeMissing) estimatedRemainingMissing = Math.round(estimatedRemainingMissing * 0.22);

  let estimatedRemainingDuplicates = duplicateRows;
  if (config.removeDuplicates) estimatedRemainingDuplicates = Math.round(estimatedRemainingDuplicates * 0.06);

  let estimatedRemainingOutliers = outlierRows;
  if (config.handleOutliers) estimatedRemainingOutliers = Math.round(estimatedRemainingOutliers * 0.28);
  ...
  const cleanedQuality = clamp(Number((dataset.quality + qualityDelta).toFixed(1)), 0, 99.8);
  const readiness = clamp(Math.round(cleanedQuality + (config.trainSplit >= 75 ? 2 : 0) + (config.trainSplit <= 88 ? 1 : 0)), 0, 100);

  return {
    approxMissingRows,
    duplicateRows,
    outlierRows,
    estimatedRemainingMissing,
    estimatedRemainingDuplicates,
    estimatedRemainingOutliers,
    cleanedQuality,
    readiness,
  };
};
...
export function DatasetsPage() {
  ...
  const [uploadProject, setUploadProject] = useState('Unassigned');
  const [uploadDomain, setUploadDomain] = useState<'UV Protection' | 'Surface Chemistry' | 'Sensory Science' | 'Photochemistry' | 'General'>('General');
  const [isUploading, setIsUploading] = useState(false);
  const [uploadError, setUploadError] = useState('');
  const [uploadCleaningConfig, setUploadCleaningConfig] = useState<CleaningConfig>(DEFAULT_CLEANING_CONFIG);
  const [uploadPreviewColumns, setUploadPreviewColumns] = useState<string[]>([]);
  const [uploadPreviewRows, setUploadPreviewRows] = useState<Array<Record<string, string | number>>>([]);
  const [uploadPreviewNote, setUploadPreviewNote] = useState('Select a file to preview rows and configure cleaning before import.');
  const [cleaningConfigByDataset, setCleaningConfigByDataset] = useState<Record<string, CleaningConfig>>({});
  const [isPreparingByDataset, setIsPreparingByDataset] = useState<Record<string, boolean>>({});
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const location = useLocation();

  // Handle deep-link behavior from other pages.
  useEffect(() => {
    const state = location.state as { expandedId?: string; openUpload?: boolean; projectName?: string } | null;

    if (state?.openUpload) {
      setIsUploadOpen(true);
      setUploadError('');
      if (state.projectName) {
        setUploadProject(state.projectName);
      }
    }
  ...
  const avgQuality = datasets.length ? (datasets.reduce((sum, dataset) => sum + dataset.quality, 0) / datasets.length).toFixed(1) : '0.0';
  const totalStorage = datasets.reduce((sum, dataset) => sum + dataset.sizeMB, 0).toFixed(1);
  ...
  const getCleaningConfig = (id: string): CleaningConfig => cleaningConfigByDataset[id] || DEFAULT_CLEANING_CONFIG;

  const setCleaningConfig = (id: string, updater: (config: CleaningConfig) => CleaningConfig) => {
    setCleaningConfigByDataset((prev) => {
      const current = prev[id] || DEFAULT_CLEANING_CONFIG;
  ...
  const resetUploadForm = () => {
    setUploadFile(null);
    setUploadName('');
    setUploadProject('Unassigned');
    setUploadDomain('General');
    setUploadError('');
    setUploadCleaningConfig(DEFAULT_CLEANING_CONFIG);
    setUploadPreviewColumns([]);
    setUploadPreviewRows([]);
    setUploadPreviewNote('Select a file to preview rows and configure cleaning before import.');
    setIsDragOver(false);
  ...
  const handleSelectedFile = async (file: File | null) => {
    if (!file) return;
    const format = getUploadFormat(file.name);

    if (!format) {
      setUploadError('Unsupported file type. Use CSV, XLSX, XLS, Parquet, or JSON.');
      setUploadFile(null);
      setUploadPreviewColumns([]);
      setUploadPreviewRows([]);
      setUploadPreviewNote('Preview unavailable for this file type.');
      return;
    }
    ...
    if (format === 'csv') {
      try {
        ...
        if (parsed.columns.length) {
          setUploadPreviewColumns(parsed.columns);
          setUploadPreviewRows(parsed.rows);
          setUploadPreviewNote(`Previewing first ${parsed.rows.length} row(s) from ${file.name}.`);
        } else {
          setUploadPreviewColumns([]);
          setUploadPreviewRows([]);
          setUploadPreviewNote('No rows detected in file.');
        }
      } catch {
        setUploadPreviewColumns([]);
        setUploadPreviewRows([]);
        setUploadPreviewNote('Could not parse CSV preview. You can still import and clean after upload.');
      }
      return;
    }

    const fallbackColumns = ['record_id', 'feature_1', 'feature_2', 'target'];
    setUploadPreviewColumns(fallbackColumns);
    setUploadPreviewRows([
      { record_id: 'R-0001', feature_1: '12.4', feature_2: '4.3', target: '42.1' },
      { record_id: 'R-0002', feature_1: '11.7', feature_2: '6.8', target: '39.6' },
      { record_id: 'R-0003', feature_1: '9.1', feature_2: '5.1', target: '35.2' },
    ]);
    setUploadPreviewNote(`Preview is inferred for ${format.toUpperCase()} files. Full profiling runs after import.`);
  };
  ...
  const completeUpload = () => {
    ...
    const format = getUploadFormat(uploadFile.name);
    const qualityDelta = getCleaningQualityDelta(uploadCleaningConfig);

    setTimeout(() => {
      const sizeMB = Number((uploadFile.size / (1024 * 1024)).toFixed(2));
      const rowEstimate = Math.max(50, Math.round(uploadFile.size / 220));
      const columnEstimate = format === 'parquet' ? 24 : format === 'xlsx' || format === 'xls' ? 16 : format === 'json' ? 12 : 10;
      const timestamp = Date.now();
      const newId = `ds-upload-${timestamp}`;
      const quality = clamp(Number((92.8 + qualityDelta).toFixed(1)), 0, 99.8);

      const previewRows = uploadPreviewRows.length
        ? uploadPreviewRows
        : [
            { record_id: 'R-0001', feature_1: 12.4, feature_2: 4.3, target: 42.1, source_file: uploadFile.name },
            { record_id: 'R-0002', feature_1: 11.7, feature_2: 6.8, target: 39.6, source_file: uploadFile.name },
            { record_id: 'R-0003', feature_1: 9.1, feature_2: 5.1, target: 35.2, source_file: uploadFile.name },
            { record_id: 'R-0004', feature_1: 14.6, feature_2: 7.9, target: 48.3, source_file: uploadFile.name },
            { record_id: 'R-0005', feature_1: 10.2, feature_2: 4.7, target: 37.0, source_file: uploadFile.name },
          ];
      ...
      const newDataset: Dataset = {
        ...
        domain: uploadDomain,
        domainColor: UPLOAD_DOMAIN_COLORS[uploadDomain] ?? '#64748b',
        description: `Uploaded from ${uploadFile.name}. Schema and preview are inferred and should be validated before modeling.`,
        rows: rowEstimate,
        ...
        tags: qualityDelta > 0 ? ['uploaded', format, 'prepared'] : ['uploaded', format],
        lastUpdated: 'Just now',
        created: formatUploadedDate(),
        project: uploadProject.trim() || 'Unassigned',
        status: qualityDelta > 0 ? 'ready' : 'processing',
        schema: [
          { name: 'record_id', type: 'string', role: 'ID', completeness: 100, sample: 'R-0001' },
          { name: 'feature_1', type: 'float', role: 'Input', completeness: 98.5, sample: '12.4', min: '0', max: '100' },
          { name: 'feature_2', type: 'float', role: 'Input', completeness: 97.2, sample: '4.3', min: '0', max: '100' },
          { name: 'target', type: 'float', role: 'Output', completeness: 95.8, sample: '42.1', min: '0', max: '100' },
          { name: 'source_file', type: 'string', role: 'Metadata', completeness: 100, sample: uploadFile.name },
        ],
        preview: previewRows,
        qualityBreakdown: [
          { label: 'Completeness', value: clamp(91.1 + (uploadCleaningConfig.imputeMissing ? 4 : 0), 0, 100), color: '#3F98FF' },
          { label: 'Validity', value: clamp(94.6 + (uploadCleaningConfig.handleOutliers ? 2 : 0), 0, 100), color: '#059669' },
          { label: 'Consistency', value: clamp(92.8 + (uploadCleaningConfig.removeDuplicates ? 3 : 0), 0, 100), color: '#7c3aed' },
          { label: 'Accuracy', value: clamp(92.7 + (uploadCleaningConfig.normalizeNumeric || uploadCleaningConfig.encodeCategoricals ? 2 : 0), 0, 100), color: '#ea580c' },
        ],
      };

      setDatasets((prev) => [newDataset, ...prev]);
      setActiveTab((prev) => ({ ...prev, [newId]: 'preview' }));
      setExpandedId(newId);
      setIsUploadOpen(false);
      resetUploadForm();
    }, 1200);
  ...
  const uploadQualityDelta = getCleaningQualityDelta(uploadCleaningConfig);
  const uploadBaseQuality = 92.8;
  const uploadEstimatedQuality = clamp(Number((uploadBaseQuality + uploadQualityDelta).toFixed(1)), 0, 99.8);
  const uploadEstimatedRows = uploadFile ? Math.max(50, Math.round(uploadFile.size / 220)) : 0;
  const uploadEstimatedMissing = Math.round(uploadEstimatedRows * 0.06 * (uploadCleaningConfig.imputeMissing ? 0.2 : 1));
  const uploadEstimatedDuplicates = Math.round(uploadEstimatedRows * 0.012 * (uploadCleaningConfig.removeDuplicates ? 0.1 : 1));
  const uploadEstimatedOutliers = Math.round(uploadEstimatedRows * 0.018 * (uploadCleaningConfig.handleOutliers ? 0.3 : 1));
  const uploadReadiness = clamp(Math.round(uploadEstimatedQuality + (uploadCleaningConfig.trainSplit >= 75 ? 2 : 0) + (uploadCleaningConfig.trainSplit <= 88 ? 1 : 0)), 0, 100);
  const isUploadMetadataReady = Boolean(uploadName.trim() && uploadProject.trim());
  const currentUploadStep = !uploadFile ? 1 : !isUploadMetadataReady ? 2 : 3;
  const uploadSteps = [
    { id: 1, title: 'Upload File', detail: 'Choose source data file' },
    { id: 2, title: 'Set Metadata', detail: 'Name, project, and domain' },
    { id: 3, title: 'Preview & Clean', detail: 'Inspect rows and options' },
    { id: 4, title: 'Import', detail: 'Finalize dataset entry' },
  ];

  const applyPreparation = (dataset: Dataset) => {
    const config = getCleaningConfig(dataset.id);
    const metrics = derivePrepMetrics(dataset, config);

    setIsPreparingByDataset((prev) => ({ ...prev, [dataset.id]: true }));
    ...
    setTimeout(() => {
      setDatasets((prev) =>
        prev.map((item) => {
          ...
          return {
            ...item,
            quality: metrics.cleanedQuality,
            status: 'ready',
            lastUpdated: 'Just now',
            description: `${item.description} Data prep pipeline applied for model training readiness.`,
            tags: item.tags.includes('prepared') ? item.tags : [...item.tags, 'prepared'],
            qualityBreakdown: [
              {
                label: 'Completeness',
                value: clamp(item.qualityBreakdown[0]?.value + (config.imputeMissing ? 4 : 0), 0, 100),
                color: '#3F98FF',
              },
              {
                label: 'Validity',
                value: clamp(item.qualityBreakdown[1]?.value + (config.handleOutliers ? 2 : 0), 0, 100),
                color: '#059669',
              },
              {
                label: 'Consistency',
                value: clamp(item.qualityBreakdown[2]?.value + (config.removeDuplicates ? 3 : 0), 0, 100),
                color: '#7c3aed',
              },
              {
                label: 'Accuracy',
                value: clamp(item.qualityBreakdown[3]?.value + (config.normalizeNumeric || config.encodeCategoricals ? 2 : 0), 0, 100),
                color: '#ea580c',
              },
            ],
          };
      ...
      setIsPreparingByDataset((prev) => ({ ...prev, [dataset.id]: false }));
      setTab(dataset.id, 'quality');
    }, 900);
  };
  ...
  return (
    <div className="flex-1 overflow-auto bg-gray-50 h-full">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4 sticky top-0 z-10">
        ...
        <div className="flex gap-6">
          {[
            ...
            { label: 'Total Records', value: totalRows.toLocaleString(), icon: Table2, color: '#7c3aed' },
            { label: 'Avg. Quality Score', value: avgQuality + '%', icon: CheckCircle2, color: '#059669' },
            { label: 'Storage Used', value: totalStorage + ' MB', icon: FileSpreadsheet, color: '#ea580c' },
          ].map((s) => (
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {ds.status === 'processing' && <Clock className="w-3.5 h-3.5 text-blue-500 shrink-0" />}
                      </div>
                ...
                {isExpanded && (
                  <div className="border-t border-gray-100 bg-gray-50">
                    ...
                    <div className="flex border-b border-gray-200 px-5 bg-white">
                      {(['schema', 'preview', 'prepare', 'quality'] as ActiveTab[]).map((t) => (
                        <button
                          key={t}
                        ...
                        >
                          ...
                          {t === 'prepare' && <Filter className="w-3 h-3" />}
                    ...
                    <div className="px-5 py-4">
                      ...
                      {tab === 'prepare' && (() => {
                        const config = getCleaningConfig(ds.id);
                        const metrics = derivePrepMetrics(ds, config);
                        const isPreparing = Boolean(isPreparingByDataset[ds.id]);
                        const trainColumns = ds.schema.filter((column) => column.role === 'Input').length;
                        const targetColumns = ds.schema.filter((column) => column.role === 'Output').length;

                        const preparationSteps: Array<{
                          key: keyof CleaningConfig;
                          label: string;
                          detail: string;
                          effect: string;
                        }> = [
                          {
                            ...
                            detail: 'Detect duplicate records using ID and feature fingerprinting.',
                            effect: `~${metrics.duplicateRows} rows identified`,
                          },
                          {
                            key: 'imputeMissing',
                            label: 'Impute Missing Values',
                            detail: 'Median for numeric fields and mode for categorical fields.',
                            effect: `~${metrics.approxMissingRows} rows impacted`,
                          },
                          {
                            key: 'handleOutliers',
                            label: 'Handle Outliers',
                            detail: 'Winsorize to robust quantile limits for stable model behavior.',
                            effect: `~${metrics.outlierRows} outlier rows`,
                          },
                          {
                            key: 'normalizeNumeric',
                            label: 'Normalize Numeric Features',
                            detail: 'Apply z-score scaling across numeric input columns.',
                            effect: 'Improves convergence and comparability',
                          },
                          {
                            key: 'encodeCategoricals',
                            label: 'Encode Categorical Features',
                            detail: 'Apply one-hot encoding to categorical input features.',
                            effect: 'Required for most ML estimators',
                          },
                        ];
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-[1.5fr_1fr] gap-6">
                            <div className="space-y-3">
                              <div className="rounded-xl border border-gray-200 bg-white p-4">
                                <div className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-3">Cleaning Pipeline</div>
                                <div className="space-y-2.5">
                                  {preparationSteps.map((step) => (
                                    <label key={step.key} className="flex items-start gap-2.5 p-2 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
                                      <input
                                        type="checkbox"
                                        checked={Boolean(config[step.key])}
                                        onChange={(e) =>
                                          setCleaningConfig(ds.id, (prev) => ({
                                            ...prev,
                                            [step.key]: e.target.checked,
                                          }))
                                        }
                                        className="mt-0.5 h-4 w-4 rounded border-gray-300 text-[#3F98FF] focus:ring-[#3F98FF]/30"
                                      />
                                      <div>
                                        <div className="text-sm font-medium text-gray-800">{step.label}</div>
                                        <div className="text-xs text-gray-500">{step.detail}</div>
                                        <div className="text-[11px] text-[#3F98FF] mt-0.5">{step.effect}</div>
                                      </div>
                                    </label>
                                  ))}
                                </div>
                              ...
                              <div className="rounded-xl border border-gray-200 bg-white p-4">
                                <div className="flex items-center justify-between mb-2">
                                  <span className="text-xs font-semibold uppercase tracking-wider text-gray-500">Train / Validation Split</span>
                                  <span className="text-xs font-semibold text-gray-700">{config.trainSplit}% / {100 - config.trainSplit}%</span>
                                </div>
                                <input
                                  type="range"
                                  min={60}
                                  ...
                                  onChange={(e) => setCleaningConfig(ds.id, (prev) => ({ ...prev, trainSplit: Number(e.target.value) }))}
                                  className="w-full accent-[#3F98FF]"
                                />
                                <div className="mt-2 text-[11px] text-gray-500">Recommended default is 80/20 for robust validation without starving training data.</div>
                              </div>
                            </div>

                            <div className="rounded-xl border border-gray-200 bg-white p-4 h-fit">
                              <div className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-3">Training Readiness</div>
                              <div className="space-y-3">
                                <div>
                                  <div className="flex items-center justify-between mb-1">
                                    <span className="text-xs text-gray-500">Readiness Score</span>
                                    <span className="text-sm font-bold text-gray-800">{metrics.readiness}%</span>
                                  </div>
                                  <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                                    <div
                                      className="h-full rounded-full"
                                      style={{
                                        ...
                                        backgroundColor: metrics.readiness >= 95 ? '#059669' : metrics.readiness >= 85 ? '#3F98FF' : '#d97706',
                                      }}
                                    />
                                  </div>
                                </div>
                                ...
                                <div className="space-y-2 text-xs">
                                  <div className="flex items-center justify-between"><span className="text-gray-500">Estimated Quality After Prep</span><span className="font-semibold text-gray-700">{metrics.cleanedQuality}%</span></div>
                                  <div className="flex items-center justify-between"><span className="text-gray-500">Features / Targets</span><span className="font-semibold text-gray-700">{trainColumns} / {targetColumns || 1}</span></div>
                                  <div className="flex items-center justify-between"><span className="text-gray-500">Remaining Missing Rows</span><span className="font-semibold text-gray-700">{metrics.estimatedRemainingMissing}</span></div>
                                  <div className="flex items-center justify-between"><span className="text-gray-500">Remaining Duplicates</span><span className="font-semibold text-gray-700">{metrics.estimatedRemainingDuplicates}</span></div>
                                  <div className="flex items-center justify-between"><span className="text-gray-500">Remaining Outliers</span><span className="font-semibold text-gray-700">{metrics.estimatedRemainingOutliers}</span></div>
                                </div>

                                <button
                                  onClick={() => applyPreparation(ds)}
                                  className="w-full mt-2 px-3 py-2 rounded-lg bg-[#3F98FF] text-white text-xs font-semibold hover:bg-[#2563eb] transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                                  disabled={isPreparing}
                                >
                                  {isPreparing ? 'Applying Pipeline...' : 'Apply Cleaning Pipeline'}
                                </button>
                              </div>
                            </div>
                          </div>
      ...
      {isUploadOpen && (
        <div className="fixed inset-0 z-40 bg-black/35 backdrop-blur-[1px] flex items-center justify-center p-4" onClick={handleUploadClose}>
          <div className="w-full max-w-[820px] max-h-[90vh] bg-white border border-gray-200 rounded-xl shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="h-14 px-5 border-b border-gray-200 flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-gray-900">Import Dataset</div>
                <div className="text-xs text-gray-500">Upload data and add it to your Data Library</div>
              </div>
              <button
            ...
            <div className="px-5 py-3 border-b border-gray-100 bg-gray-50/70">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {uploadSteps.map((step) => {
                  const isDone = step.id < currentUploadStep;
                  const isCurrent = step.id === currentUploadStep;
                  return (
                    <div
                      key={step.id}
                      className={`rounded-lg border px-2.5 py-2 transition-colors ${
                        isCurrent
                          ? 'border-[#3F98FF]/40 bg-[#3F98FF]/10'
                          : isDone
            ...
            <div className="p-5 space-y-4 overflow-y-auto">
              ...
              <button
                ...
                className={`w-full border-2 border-dashed rounded-xl p-6 text-left transition-colors ${
                  isDragOver ? 'border-[#3F98FF] bg-[#3F98FF]/5' : 'border-gray-300 hover:border-[#3F98FF]/50'
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-[#3F98FF]/10 flex items-center justify-center">
                    <FileUp className="w-5 h-5 text-[#3F98FF]" />
                  </div>
                  <div>
                    <div className="text-sm font-semibold text-gray-900">Drop a data file here or click to browse</div>
                    <div className="text-xs text-gray-500 mt-0.5">Supported: CSV, XLSX, XLS, Parquet, JSON</div>
                  </div>
                </div>
                {uploadFile && (
                  <div className="mt-4 rounded-lg bg-gray-50 border border-gray-200 px-3 py-2 text-xs text-gray-600">
                    <span className="font-medium text-gray-800">Selected:</span> {uploadFile.name} ({(uploadFile.size / (1024 * 1024)).toFixed(2)} MB)
                  </div>
                )}
              </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <label className="block">
                  <span className="text-xs font-medium text-gray-600">Dataset Name</span>
                  <input
                    type="text"
                    value={uploadName}
                    onChange={(e) => setUploadName(e.target.value)}
                    placeholder="e.g. Q1 Pilot Formulation Data"
              ...
              <label className="block">
                ...
                <select
                  value={uploadDomain}
                  onChange={(e) => setUploadDomain(e.target.value as 'UV Protection' | 'Surface Chemistry' | 'Sensory Science' | 'Photochemistry' | 'General')}
                  className="mt-1 w-full px-3 py-2 text-sm border border-gray-300 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#3F98FF]/30 focus:border-[#3F98FF]"
                >
              ...
              {uploadFile && (
                <div className="space-y-4">
                  <div className="min-w-0 rounded-xl border border-gray-200 bg-white p-4 overflow-hidden">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-semibold uppercase tracking-wider text-gray-500">Step 3: Data Preview</span>
                      <span className="text-[11px] text-gray-400">{uploadPreviewRows.length} row(s)</span>
                    </div>
                    <div className="text-[11px] text-gray-500 mb-2">{uploadPreviewNote}</div>

                    {uploadPreviewColumns.length > 0 ? (
                      <div className="max-w-full overflow-x-auto border border-gray-200 rounded-lg">
                        <table className="min-w-max text-xs bg-white">
                          <thead>
                            <tr className="bg-gray-50 border-b border-gray-200">
                              {uploadPreviewColumns.map((column) => (
                                <th key={column} className="text-left py-2 px-2 font-semibold text-gray-500 whitespace-nowrap">{column}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody>
                            {uploadPreviewRows.map((row, idx) => (
                              <tr key={idx} className="border-b border-gray-100 last:border-0">
                                {uploadPreviewColumns.map((column) => (
                                  <td key={`${idx}-${column}`} className="py-2 px-2 text-gray-700 font-mono whitespace-nowrap">{String(row[column] ?? '')}</td>
                                ))}
                              </tr>
                            ))}
                          </tbody>
                      ...
                      <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-4 text-xs text-gray-500">No preview available yet.</div>
                    )}
                  </div>

                  <div className="min-w-0 rounded-xl border border-gray-200 bg-white p-4">
                    <div className="text-xs font-semibold uppercase tracking-wider text-gray-500 mb-3">Step 3: Clean Before Import</div>
                    ...
                    <div className="space-y-2 mb-4">
                      {([
                        ...
                        ['normalizeNumeric', 'Normalize numeric fields'],
                        ['encodeCategoricals', 'Encode categorical fields'],
                      ] as Array<[keyof CleaningConfig, string]>).map(([key, label]) => (
                        <label key={key} className="flex items-center gap-2 text-xs text-gray-700 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={Boolean(uploadCleaningConfig[key])}
                            onChange={(e) => setUploadCleaningConfig((prev) => ({ ...prev, [key]: e.target.checked }))}
                            className="h-4 w-4 rounded border-gray-300 text-[#3F98FF] focus:ring-[#3F98FF]/30"
                          />
                          {label}
                        </label>
                      ))}
                    ...
                    <div className="mb-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-xs text-gray-500">Train / Validation</span>
                        <span className="text-xs font-semibold text-gray-700">{uploadCleaningConfig.trainSplit}% / {100 - uploadCleaningConfig.trainSplit}%</span>
                      </div>
                      <input
                        type="range"
                        min={60}
                        max={90}
                        value={uploadCleaningConfig.trainSplit}
                        onChange={(e) => setUploadCleaningConfig((prev) => ({ ...prev, trainSplit: Number(e.target.value) }))}
                        className="w-full accent-[#3F98FF]"
                      />
                    </div>

                    <div className="space-y-1.5 text-xs border-t border-gray-100 pt-3">
                      <div className="flex items-center justify-between"><span className="text-gray-500">Estimated Quality</span><span className="font-semibold text-gray-700">{uploadEstimatedQuality}%</span></div>
                      <div className="flex items-center justify-between"><span className="text-gray-500">Readiness</span><span className="font-semibold text-gray-700">{uploadReadiness}%</span></div>
                      <div className="flex items-center justify-between"><span className="text-gray-500">Remaining Missing Rows</span><span className="font-semibold text-gray-700">{uploadEstimatedMissing}</span></div>
                      <div className="flex items-center justify-between"><span className="text-gray-500">Remaining Duplicates</span><span className="font-semibold text-gray-700">{uploadEstimatedDuplicates}</span></div>
                      <div className="flex items-center justify-between"><span className="text-gray-500">Remaining Outliers</span><span className="font-semibold text-gray-700">{uploadEstimatedOutliers}</span></div>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="h-14 px-5 border-t border-gray-200 flex items-center justify-end gap-2">
              <button
                onClick={handleUploadClose}
